...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    attributeChangedCallback() {
      // Hide the footer immediately when no-overlay is toggled on mid-show;
      // _flashOverlay's guard keeps it from reappearing on mouse/nav.
      if (this._overlay && this.hasAttribute('no-overlay')) {
    ...
    _restoreIndex() {
      ...
      // state across loads.
      // Read the hash ONCE and keep the target pending: slotchange can fire
      // while the parser has only streamed part of the document, and
      // _applyIndex rewrites location.hash to the current slide — so
      // re-reading the hash on a later slotchange would see the clobbered
      // value (#1), not the deep-link. Pending until the target slide has
      // been parsed, then consumed so it can't override user navigation.
      if (this._deepLink === undefined) {
        const h = (location.hash || '').match(/^#(\d+)$/);
        this._deepLink = h ? parseInt(h[1], 10) - 1 : null;
      }
      if (this._deepLink != null && this._deepLink >= 0 && this._deepLink < this._slides.length) {
        this._index = this._deepLink;
        this._deepLink = null;
      }
    }
    ...
    _flashOverlay() {
      ...
      // flash) so the audience sees clean slides. The `no-overlay` attribute
      // is an author-controlled equivalent: a deck can hide the footer by
      // default and reveal it on demand (e.g. a toggle key).
