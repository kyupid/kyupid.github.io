...
## 객체를 캡슐화 안하면 디버깅 힘듦
- DB에 저장하기전 복잡한 로직이 있다고 가정해보자.
- 객체 생성할때는 A {123} 이었는데, 저장후 확인해보니 A {456} 이라고 해보자.
- 생성후에 필드 변경하는 곳이 많다면 어떤 맥락에서 변경이 되었는지 전혀 알 수 없음.
- 그래서 디버깅하기가 굉장히 지옥이다. 버그 발생하면 전후로 로그 찍고 상태 변경을 추적할 수 밖에없음
- 객체를 캡슐화하고 상태 변경에 대한 예외 처리나 메소드 이름으로 맥락을 제공하면, 디버깅하기가 쉬움[]()
